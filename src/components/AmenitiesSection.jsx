...
export default function AmenitiesSection({ theme, currentLang }) {
  ...
  return (
    <section id="comodidades" className={`py-16 lg:py-24 transition-colors duration-400 ${
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {AMENITIES.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`p-6 rounded-xl space-y-4 group cursor-pointer transition-all duration-300 border ${
                  isDark 
              ...
              >
                <div className="w-12 h-12 rounded-lg bg-[#ac926f]/20 border border-[#ac926f]/40 flex items-center justify-center text-[#ac926f] group-hover:bg-[#ac926f] group-hover:text-white transition-colors">
                  <IconComp className="w-6 h-6" />
                </div>

                <div className="space-y-1.5">
                  <h3 className={`font-serif-luxury text-xl font-bold transition-colors ${
