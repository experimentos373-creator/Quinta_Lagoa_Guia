...
export default function VillaOverviewSection({ onOpenBooking, theme, currentLang }) {
  ...
  return (
    <section id="overview" className={`py-16 lg:py-24 transition-colors duration-400 border-t border-b ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <h2 className={`text-3xl sm:text-5xl font-serif-luxury font-normal leading-tight ${
            ...
            <div className={`p-6 sm:p-8 rounded-2xl border shadow-2xl space-y-5 transition-all ${
              isDark ? 'bg-[#2b2829] border-white/10 text-white' : 'bg-white border-stone-200 text-[#1a1919]'
            }`}>
              <h3 className="font-serif-luxury text-lg font-bold text-[#ac926f] flex items-center gap-2">
              ...
              <form onSubmit={handleCheckAvailability} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="sm:col-span-3 pt-2">
                  <button
                    type="submit"
                    className="w-full gold-btn font-bold text-xs uppercase tracking-wider py-3.5 rounded-xl shadow-lg flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-95 transition-all"
                  >
          ...
          <div className="lg:col-span-6">
            <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-stone-500/20 group">
              ...
              <div className="aspect-[4/3] w-full overflow-hidden relative bg-black/40">
                ...
                {/* Slide Caption */}
                <div className="absolute bottom-4 inset-x-4 p-4 text-center">
                  <span className="font-serif-luxury text-base sm:text-lg font-bold text-white tracking-wide drop-shadow-md">
              ...
              <button
                onClick={prevSlide}
                className="absolute left-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-black/60 text-white hover:bg-[#ac926f] transition-all border border-white/20"
                title="Foto Anterior"
              >
              ...
              <button
                onClick={nextSlide}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-2.5 rounded-full bg-black/60 text-white hover:bg-[#ac926f] transition-all border border-white/20"
                title="Próxima Foto"
              >
              ...
              <div className="absolute bottom-2 inset-x-0 flex justify-center gap-1.5 pb-2">
                {carouselImages.map((_, idx) => (
                  <button
                    key={idx}
                    onClick={() => setCurrentSlide(idx)}
                    className={`h-1.5 rounded-full transition-all ${
                      currentSlide === idx ? 'w-6 bg-[#e7d49d]' : 'w-1.5 bg-white/50'
                    }`}
                  ></button>
                ))}
              </div>
